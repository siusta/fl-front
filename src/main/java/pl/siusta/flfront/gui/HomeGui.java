...
import pl.siusta.flfront.model.Flashcard;
import pl.siusta.flfront.model.FlashcardList;
import pl.siusta.flfront.service.FlashcardService;
...
public class HomeGui extends VerticalLayout{
    ...
    @Autowired
    public HomeGui(FlashcardService fService) throws JsonProcessingException {
        this.fService = fService;
        List<FlashcardList> flashcardLists = fService.getFlashcardsLists();
    ...
    public HorizontalLayout oneList(List<FlashcardList> flashcardLists,FlashcardList f){
        ...
        Button view = new Button("View", buttonClickEvent -> {
            try {
                viewWords("All word pairs:",dataSetUp(flashcardLists,f.getId()));
        ...
        Button learn = new Button("Learn",buttonClickEvent -> {
            try {
                learn(dataSetUp(flashcardLists,f.getId()));
            } catch (JsonProcessingException e) {
                e.printStackTrace();
            }
        });
        Button exercise = new Button("Exercise", buttonClickEvent -> {
            try {
                exercise(dataSetUp(flashcardLists,f.getId()));
            } catch (JsonProcessingException e) {
                e.printStackTrace();
            }
        });
    ...
    public List<HorizontalLayout> allLists(List<FlashcardList> flashcardLists){
        ...
        for (FlashcardList f: flashcardLists
            ...
            all.add(oneList(flashcardLists,f));
        }
        return all;
    }

    public List<Flashcard> dataSetUp(List<FlashcardList> flashcardLists, Long id) throws JsonProcessingException {
        FlashcardList flashcardList = getFListById(flashcardLists,id);
    ...
    public List<Flashcard> getFlashcardsFromJson(String fjson) throws JsonProcessingException {
        ObjectMapper objectMapper = new ObjectMapper();
        List<Flashcard> flashcards = objectMapper.readValue(fjson, new TypeReference<List<Flashcard>>() {
        });
        return flashcards;
    }

    public FlashcardList getFListById(List<FlashcardList> all,Long id){
        for(int i=0;i<all.size();i++){
            if(all.get(i).getId()==id)return all.get(i);
        }
        return null;
    }
